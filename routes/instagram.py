...
async def generate_caption(request: CaptionRequest):
    ...
    - **tone**: The tone of the caption (casual, professional, energetic, etc.)
    - **length**: The length of the caption (short, medium, long)
    """
    try:
        caption = await content_generator.generate_instagram_caption(
            topic=request.topic,
...
async def generate_hashtags(request: HashtagRequest):
    ...
    """
    try:
        hashtags = await content_generator.generate_instagram_hashtags(
            topic=request.topic,
            count=request.count
        )
...
async def generate_content_ideas(request: ContentIdeasRequest):
    ...
    """
    try:
        ideas = await content_generator.generate_instagram_content_ideas(
            niche=request.niche,
            count=request.count
        )
